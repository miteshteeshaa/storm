...
const { resizeLobbySheet } = require(’../../utils/sheets’);
...
? sessions.map((s, i) => `**${i + 1}.** \`${s.id}` — ${s.name}`).join('\n') : '*No sessions configured yet.*'; return new EmbedBuilder() .setColor(0x5865F2) .setTitle('⚙️ Sessions') .setDescription(`You have **${sessions.length}/${MAX_SESSIONS}** sessions configured.\n\n${lines}`)
...
name: ‘🎮 Scrim Settings’,
value: [
`🏆 Name: \`${settings.scrim_name}``, `🏟️ Lobbies: `${numLobbies}``, `🎯 Slots Per Lobby: `${slotsPerLobby}``, `📊 Total Slots: `${numLobbies * slotsPerLobby}``, `🔢 First Slot: `${settings.first_slot}``, ].join('\n'), inline: false, }, { name: '📢 Session Channels', value: [ `📝 Registration: ${ch(sessionCfg.register_channel)}`, `🎯 Slot Allocation: ${ch(sessionCfg.slotlist_channel)}`, `📸 Match Results: ${ch(sessionCfg.match_channel)}`, ].join('\n'), inline: false, }, { name: '🏟️ Lobby Channels & Roles', value: lobbyLines || '—', inline: false }, { name: '📊 Google Sheet', value: sessionCfg.sheet_url ? `[Open Sheet](${sessionCfg.sheet_url})` : '`Not Set`’, inline: false },
)
.setTimestamp();
}
...
while (true) {
  ...
  if (value === 'create_session') {
    ...
    // Redeploy guild commands so /clear shows updated session choices
    const { deployCommandsForGuild } = require('../../handlers/commandHandler');
    deployCommandsForGuild(interaction.client, guildId, freshSessions()).catch(() => {});
...
if (value === 'delete_session') {
  ...
  if (j.customId === 'confirm_delete') {
    ...
    // Redeploy guild commands so /clear shows updated session choices
    const { deployCommandsForGuild } = require('../../handlers/commandHandler');
    deployCommandsForGuild(interaction.client, guildId, getSessions(guildId)).catch(() => {});
...
if (['scrim_name','lobbies','slots_per_lobby','first_slot'].includes(value)) {
  ...
  if (['lobbies','slots_per_lobby','first_slot'].includes(value)) {
    ...
    // Auto-resize sheet if slots_per_lobby increased
    if (value === 'slots_per_lobby') {
      const sessionCfg = getSessionConfig(guildId, sessionId);
      if (sessionCfg.spreadsheet_id) {
        const lobbyConf    = fresh().cfg ? {} : {};
        const { settings: stgNow } = fresh();
        const numLobbies   = stgNow.lobbies || 4;
        ...
        (async () => {
          for (const letter of lobbyLetters) {
            try {
              await resizeLobbySheet(sessionCfg.spreadsheet_id, letter, num);
              console.log(`✅ Resized Lobby ${letter} to ${num} slots`);
            } catch (e) {
              console.error(`⚠️ Resize Lobby ${letter} failed:`, e.message);
            }
          }
        })();
      }
...
// ── Session channels (register_channel, slotlist_channel, match_channel) ────────────
if (['register_channel','slotlist_channel','match_channel'].includes(value)) {
  const labelMap = { register_channel: 'Registration Channel', slotlist_channel: 'Slot Allocation Channel', match_channel: 'Match Results Channel' };
